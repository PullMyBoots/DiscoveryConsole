...
编写 grader 前，与用户选择一个级别。这是任务级研究设计契约，不是运行时偏好。研究问题、部署场景和预期主张一旦明确，这个问题理论上就应当按设计匹配一个级别；对固定任务而言，L1/L2/L3 是备选项，不是三个同时存在的模式，也不是试错档位。

主要判据是目标环境的确定性。目标场景越固定、封闭、完全定义，越适合 L1。目标环境越不确定、开放、依赖真实部署条件，越适合 L3。

- L1：固定/开放场景。A-space 计分机制对 agent 公开。适用于直接针对已知目标优化的问题：加速固定组件、改进固定 benchmark 路径，或在完全定义的契约下优化程序。仍然需要正确性和资源护栏，但不存在独立的隐藏泛化主张。
- L2：固定场景但存在过拟合风险。适用于评估传统足够成熟、能够生成或保留代表性隐藏数据的任务，例如基于模拟的统计方法、稳定算法 benchmark 或固定任务族。Agent 在 A-space 探索，隐藏 B-space 决定排名或接受，以防止对公开 probe 过拟合。
- L3：开放世界或部署不确定验证。适用于 A/B 证据有助于搜索、但不足以完全支撑最终主张的任务。B-space 上最好的 candidate 仍可能过拟合验证体系或陷入局部最优。C-space 必须封闭，并在 CORAL 运行后用于最终人类/Codex 验证；常规 agent eval loop 不应暴露或运行 C-space。

所选级别应写入 `knowledge/eval_spec.md`，并说明该任务的允许 agent API 和隐藏边界。

如果主张涉及泛化，A/B/C 空间应当构成逐级增强的证据阶梯，而不是从同一分布中做朴素随机切分：

- A-space 应便宜、开放、简单到足以提供优化信号，但不能简单到像玩具问题，导致进展无法迁移。
- B-space 应保持隐藏，并且比 A 更接近目标场景，同时与 A 保持足够连续性，使 A 上的改进有机会迁移。
- C-space 在使用时应最接近真实部署环境或最终科学主张，并在最终验证前保持封闭。

A 与 B、B 与 C 之间的差距应当是有意设计的，但不能断档。A 太容易或太不像 B，agent 就学不到有效方向；B 太不像 C，选出的 winner 可能正好在最需要可信的地方失败。

不要把同一个研究问题做成“先试 L1，再试 L2，再试 L3”。如果评估级别发生变化，说明研究设计已经改变。应启动新的 task version 或 timestamp lineage，记录新的 eval contract，并避免与旧级别下产生的 attempt 直接比较分数。
