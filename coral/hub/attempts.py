...
def _global_eval_count_path(coral_dir: str | Path) -> Path:
    """Global counter path."""
    return Path(coral_dir) / "public" / "eval_count"


def increment_eval_count(coral_dir: str | Path) -> int:
    """Increment the eval counter and return the new value."""
    ...
    return _bump(_global_eval_count_path(coral_dir))
...
def search_attempts(
    ...
    sources = read_attempts(coral_dir)
