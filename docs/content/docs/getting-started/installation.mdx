...
Prepare and start them through the normal timestamp workflow — no extra `uv sync` step is required:

```bash
# SWE-bench Verified (meta-solver optimization over 500 GitHub issues)
coral prepare -c examples/swebench-verified/task.yaml
coral start -c results/<task>/latest/.coral/config.yaml

# terminal-bench (meta-solver optimization over terminal/shell tasks)
coral prepare -c examples/terminal-bench/task.yaml
coral start -c results/<task>/latest/.coral/config.yaml
```
