...
```bash
coral prepare -c my-task/task.yaml
coral validate --run-dir my-task/results/<task>/latest/.coral
coral start -c my-task/results/<task>/latest/.coral/config.yaml
```

CORAL will:
1. Prepare a timestamped `.coral/` shared state directory
2. Create isolated git worktrees for each agent
3. Generate an instruction file in each worktree
4. Launch agents from the prepared timestamp
