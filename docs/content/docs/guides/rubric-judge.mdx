...
|--------|--------|--------|----------|
| [`race_japan_grader`](https://github.com/PullMyBoots/DiscoveryConsole/tree/main/examples/race-japan-elderly/grader) | **Static** — criteria are fixed per task | `grader.args.rubrics` in `task.yaml` | Benchmarks where the evaluation protocol must be stable across runs and agents |
| [`apex_judge`](https://github.com/PullMyBoots/DiscoveryConsole/tree/main/examples/apex-eggshell-skull/grader) | **Dynamic** — judge auto-generates the rubric on first eval, evolves it on plateau | Agent's own reading of the task, plus optional seed hints | Exploratory tasks where you want the bar to rise as the agent improves |
...
Designed for the DeepResearch-Bench-style RACE task (see [examples/race-japan-elderly](https://github.com/PullMyBoots/DiscoveryConsole/tree/main/examples/race-japan-elderly)). The rubric is exactly what you declare — the judge neither adds nor removes criteria.
