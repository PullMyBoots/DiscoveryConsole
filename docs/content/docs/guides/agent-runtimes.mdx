...
CORAL spawns each agent as `kiro-cli chat <prompt> --no-interactive -a` (the `-a` flag trusts all tools). Instructions are read from `KIRO.md` at the worktree root. Kiro does not currently expose a session id we can resume from, so restarted agents start fresh — they still see prior attempts, knowledge indexes, and skills via the shared `.kiro/` directory.
