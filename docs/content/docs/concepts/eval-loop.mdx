...
## Work and Reflection Loops

Modern CORAL workbench runs are organized around two visible agent states:

- `work_loop`: the agent implements, diagnoses, or explores a candidate method.
- `reflect_loop`: the agent uses eval feedback and shared evidence to update
  its route before returning to work.

There is no interval or plateau heartbeat action system in the workbench loop.
Reflection is driven by completed eval reports, practice archives, and
outer-loop review notes.

## Global eval count

The file `.coral/public/eval_count` tracks the total number of evals across all agents.
