...
When you run `coral prepare`, the following structure is created. Later,
`coral start` launches agents from this prepared timestamp:
...
│   │   ├── knowledge/         # Manuals, sources, agent seed briefs, practice archive
│   │   ├── skills/            # Reusable tools/scripts
│   │   ├── jobs/              # Open A-space compute job records
│   │   ├── control/           # Dashboard/manager control state
│   │   ├── logs/              # Agent session logs
│   │   ├── eval_count         # Global eval counter
│   │   ├── grader_daemon.pid  # Daemon process ID
│   │   └── grader_daemon_liveness  # Daemon liveness timestamp
...
└── agents/
    ...
    │   ├── CORAL_SHARED       # Symlink to .claude/ or .codex/ shared state dir
