...
5. Symlinks shared state (attempts, knowledge, skills, jobs, logs) from `.coral/public/`
...
Agents share knowledge through `coral kb` indexes and skills in `.coral/public/`.
...
- Agents can access public shared state through their runtime shared directory and `CORAL_SHARED`
...
You can override the model while preparing the timestamp:

```bash
coral prepare -c task.yaml agents.model=opus
```
...
- Shared indexed practice knowledge for communicating insights
